...
are a pair: **Cline's hooks record your work automatically with full AI provenance**
(model, vendor, session, the decision graph), and these commands let you **read that
history back**. Use them to ground yourself in reality instead of guessing.

You do **not** run `atomic add` or `atomic record` — Cline's hooks do that when the
task ends. Everything in this skill is **read-only inspection**, safe to run at any
...
- You don't record (`atomic add`/`record`) — Cline's hooks do, with provenance. These
  commands let you read that provenance back.
