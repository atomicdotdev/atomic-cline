...
After editing intent markdown files, run `atomic vault sync` to persist changes back to the vault database. **Sync before every `intent show` and before every `intent update`** — the CLI reads from the database, not the file. Without a sync, `show` renders the stale placeholder template, and `update` re-materializes the database copy over the file, clobbering your edits. `atomic vault sync` is not `atomic record`; it only moves your `.vault/` edits into the database, and hooks do not do it for you.
...
Write code and iterate. You do **not** create or switch views, and you do **not** run `atomic add` or `atomic record` — Cline's hooks own all of that:
...
- **Turn end** records automatically — the hook runs `status` → `add` (tracks new files) → `record --all` with full AI provenance (model, vendor, session, timing, decision graph).
...
- Use `atomic vault sync` after editing any vault markdown files, and always before `intent show`/`update` so the CLI reads fresh state instead of clobbering your edits
- You don't manage views or recording — Cline's hooks fork a draft view at session start, record at turn end, and restore your view at session end. Inspect the results with the `atomic-vcs` skill.